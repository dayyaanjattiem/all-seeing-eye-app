...
from config import TRADING_DAYS_PER_YEAR, FORECAST_DAYS_TS
...
def run_holt_winters_forecast(price_series, forecast_days=FORECAST_DAYS_TS):
    ...
    try:
        data = price_series.to_frame(name='Close').dropna()
        ...
        for ticker in log_returns.columns:
            if ticker != index_ticker:
                ...
                if market_returns.var() > 1e-8:
                    ...
                    betas[ticker] = model.params.get(index_ticker, np.nan)
                else:
                    betas[ticker] = np.nan
    else:

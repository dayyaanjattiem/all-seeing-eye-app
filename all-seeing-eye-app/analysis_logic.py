...
def run_holt_winters_forecast(price_data, forecast_days=FORECAST_DAYS_TS):
    ...
    try:
        ...
        # Reduced minimum requirement slightly to allow for newer stocks
        if data.empty or len(data) < 10: 
            return pd.Series()

        # 2. Fit Model on NUMPY values (Bypasses 'to_frame' pandas/statsmodels index bugs)
        ...
        # 4. Reconstruct Date Index for the Forecast
        last_date = data.index[-1]
        # Use business days ('B') to skip weekends
...
# --- 4. CORRELATION & BETA ANALYSIS (FIXED) ---
def analyze_correlation_beta(df, index_ticker):
    """Calculates correlation and Beta of stocks vs a benchmark index."""
    
    # 1. Normalize
    df_filled = df.ffill().bfill()
    normalized = df_filled / df_filled.iloc[0] * 100
    
    # Calculate Correlation
    correlations = df_filled.corr()[[index_ticker]].drop(index=index_ticker)
    ...
    if index_ticker in log_returns.columns:
        ...
        if market_var > 1e-8: # Avoid division by zero
            # We rename the market column to 'Market' via numpy to avoid special char issues (e.g. ^J141.JO)
            X = sm.add_constant(market_returns.values) 
            
            for ticker in log_returns.columns:
                if ticker != index_ticker:
                    stock_returns = log_returns[ticker].values
                    try:
                        # OLS: Stock ~ Constant + Market
                        model = sm.OLS(stock_returns, X).fit()
                        # The beta is the coefficient for the Market (index 1)
                        beta = model.params[1] 
                        betas[ticker] = beta
                    except:
                        betas[ticker] = np.nan
    ...
    beta_df = pd.DataFrame.from_dict(betas, orient='index', columns=['Beta vs Index'])
    
    # Combine results and ensure numeric
    results = correlations.join(beta_df)
    results = results.apply(pd.to_numeric, errors='coerce')
    
    return normalized, results

# --- 5. PLOTTING FUNCTION (FIXED) ---
def plot_correlation_beta(normalized_df, index_ticker, corr_beta_df):
    ...
    for col in normalized_df.columns:
        clean_series = normalized_df[col].dropna()
        if not clean_series.empty:
            ax1.plot(clean_series.index, clean_series, alpha=0.7, linewidth=1.5, label=col)
    ...
    # Ensure data is strictly numeric (floats)
    corr_beta_df = corr_beta_df.apply(pd.to_numeric, errors='coerce')
    
    if len(corr_beta_df) > 0 and 'Correlation with Index' in corr_beta_df.columns:
        # fill NaNs with 0 for the plot to avoid crashing
        corr_values = corr_beta_df['Correlation with Index'].fillna(0).values.reshape(-1, 1)
        
        im = ax2.imshow(corr_values, cmap='RdYlBu_r', aspect='auto', vmin=-1, vmax=1)
        ax2.set_yticks(range(len(corr_beta_df)))
        ax2.set_yticklabels(corr_beta_df.index, fontsize=9)
        ax2.set_xticks([0])
        ax2.set_xticklabels(['Corr.'])
        ax2.set_title(f'Correlation with {index_ticker}', fontsize=14, fontweight='bold')
        ...
        ax2.text(0.5, 0.5, 'No Correlation Data', transform=ax2.transAxes, ha='center')
    ...
    if 'Beta vs Index' in corr_beta_df.columns:
        ...
        if len(valid_data) > 0:
            ...
            ax3.text(0.5, 0.5, 'No Valid Beta Data', transform=ax3.transAxes, ha='center')
            ax3.set_title('Beta vs Correlation Analysis', fontsize=14, fontweight='bold')
    ...
    if 'Beta vs Index' in corr_beta_df.columns:
        ...
        if len(beta_values) > 0:
            ax4.hist(beta_values, bins=max(2, min(10, len(beta_values))), alpha=0.7, 
                    color='lightcoral', edgecolor='black')
            ax4.axvline(x=1, color='red', linestyle='--', linewidth=2, label='Beta = 1')
            ax4.axvline(x=beta_values.mean(), color='blue', linestyle='-', linewidth=2, 
                       label=f'Avg: {beta_values.mean():.2f}')
            ax4.set_title('Beta Distribution', fontsize=14, fontweight='bold')
            ax4.legend()
        else:
             ax4.text(0.5, 0.5, 'No Beta Data', transform=ax4.transAxes, ha='center')
             ax4.set_title('Beta Distribution', fontsize=14, fontweight='bold')
